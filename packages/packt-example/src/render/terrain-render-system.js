...
export default class TerrainRenderSystem
{
    ...
    constructor(gl: any) {
        this._programInfo = twgl.createProgramInfo(gl, [positionNormalVert, diffuseSpecularFrag]);
        this._bufferInfo = twgl.primitives.createPlaneBufferInfo(gl,100,100,10,10);
    }

    worldAddingEntity(entity: Entity): void {
        entity.hasComponent(Components.TerrainComponent, terrain => this._terrain = terrain);
        entity.hasComponent(Components.CameraComponent, camera => this._camera = camera);
    }

    worldRemovingEntity(entity: Entity): void {
        entity.hasComponent(Components.TerrainComponent, terrain => this._terrain = null);
