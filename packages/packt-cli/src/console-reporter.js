...
class ConsoleReporter implements Reporter {
  ...
  onInit(version: string, options: PacktOptions) {
  ...
  onBundleWarning(bundleName: string, variant: string, warning: string) {
  ...
  _showTableRow(row: string, width: number) {
    console.log(`${VSPLIT}${S(row).padRight(width)}${VSPLIT}`);
  }
...
function printContentError(err) {
  ...
  if (index < 0) {
    ...
    err.error =
      chalk.bold(err.error.substr(0, index + 1)) + err.error.substr(index + 1);
  }
