...
import { PacktError } from 'packt-types';
...
export default class ResolverChain extends events.EventEmitter {
  ...
  constructor(config: PacktConfig) {
    ...
    this._resolvers = (config.resolvers.custom || []).map(r => {
      return new (require(r.require))(r.invariantOptions);
    });
    const resolverOptions = config.resolvers.builtIn &&
  ...
  resolve(
    ...
    const resolvedParentModule =
      (searchOptions && searchOptions.resolvedParentModule) || this._configFile;
