          ...
          (err, value: PacktConfig) => {
            ...
            for (let b in value.bundles) {
              ...
              if (typeof value.bundles[b].depends === 'string') {
                ...
                value.bundles[b].depends = (value.bundles[b]
                  .depends: any).reduce((prev, next) => {
                  prev[next] = true;
                  return prev;
                }, {});
              }
    ...
    bundlers: joi
      ...
      .pattern(
        ...
        joi.object({
          ...
          invariantOptions: joi
            .object({
              staticOutputPathFormat: joi
                .string()
                .default('/bundles/${name}_${hash}${ext}'),
            ...
            invariantOptions: joi
              .object({
                ...
                searchPaths: joi
                  .array()
                  .items(joi.string())
                  .default([workingDirectory, 'node_modules']),
                extensions: joi
                  .array()
...
function buildVariants(config: PacktConfig): Promise<PacktConfig> {
  ...
  for (let handler of config.handlers) {
    const extraVariants = Object.keys(handler.options.variants);
  ...
  for (let b in config.bundlers) {
    ...
    const extraVariants = Object.keys(bundler.options.variants);
    for (let extra of extraVariants) {
      if (!variants.find(v => v === extra)) {
        variants.push(extra);
      }
  ...
  for (let b in config.bundles) {
    ...
    const extraVariants = Object.keys(bundle.bundlerOptions.variants);
    for (let extra of extraVariants) {
      if (!variants.find(v => v === extra)) {
        variants.push(extra);
      }
